...
class Solution {
    public boolean validateStackSequences(int[] pushed, int[] popped) {
        ...
        while (idxPush != pushed.length) {
            int numPop = popped[idxPop], numPush = pushed[idxPush];
            if (numPop == numPush) {
                idxPop++;
                idxPush++;
            } else if (!stackIn.empty() && stackIn.peek() == numPop) {
                stackIn.pop();
                idxPop++;
            } else {
                stackIn.push(numPush);
                idxPush++;
            }
        }
        while (!stackIn.empty() && idxPop != popped.length && popped[idxPop] == stackIn.peek()) {
            idxPop++;
            stackIn.pop();
        }
